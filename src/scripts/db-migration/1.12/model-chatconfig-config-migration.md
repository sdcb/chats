...
- 保证 ModelSnapshot 与 ModelKeySnapshot 能完整表达一次实际请求所依赖的模型能力与传输层配置。
...
### 1.1 ModelKeySnapshot 仍缺少 key 级 custom headers / body

当前 ModelKeySnapshot 只承载：
...
它还不能承载 key 级 custom headers / custom body。

这意味着如果后续要支持“同一个模型快照叠加不同 key 级传输层配置”，就需要把这两个字段补到 ModelKeySnapshot，而不是重新塞回 ModelKey head 表。
...
- key 级连接信息与 key 级传输层补充字段，落在 ModelKeySnapshot。
- 模型能力、模型级传输层、模型默认行为，落在 ModelSnapshot。
...
### 5. 自定义 headers/body 采用轻量文本存储

对 override 请求结构，优先采用单列文本，而不是拆子表。理由是：
...
- CustomHeaders 采用多行文本，每行一个 header。
- CustomBody 采用 JSON patch object 文本。
...
- CustomHeaders：key 级额外请求头，采用多行文本，每行一个 header，建议格式为 `Header-Name: value`。
- CustomBody：key 级额外请求体 patch，语义为 JSON patch object。

### 1.2 增加到 ModelKeySnapshot 的理由

这两个字段属于 key 级传输层配置快照，不属于 ModelKey identity，也不应该加回 ModelKey head 表。

把它们落在 ModelKeySnapshot，可以和 Host、Secret 一起形成同一版本 key 的完整连接语义。
...
- CustomHeaders：模型级额外请求头，采用多行文本，每行一个 header，建议格式为 `Header-Name: value`。
- CustomBody：模型级额外请求体 patch，语义为 JSON patch object。

### 2.3 不新增到 Model head 表的理由
...
| 能力 | ModelKeySnapshot | ModelSnapshot | ChatConfig | ChatConfigSnapshot |
|---|---|---|---|---|
| 输出格式后缀名支持列表 | 否 | 是 | 否 | 否 |
| 本次输出格式选择 | 否 | 否 | 是 | 是 |
| 本次 Compression 选择 | 否 | 否 | 是 | 是 |
| effort 可选值列表 | 否 | 是 | 否 | 否 |
| effort 当前值 | 否 | 否 | 是 | 是 |
| override url | 否 | 是 | 否 | 否 |
| model 级 custom headers | 否 | 是 | 否 | 否 |
| model 级 custom body | 否 | 是 | 否 | 否 |
| key 级 custom headers | 是 | 否 | 否 | 否 |
| key 级 custom body | 是 | 否 | 否 | 否 |
...
- ModelKeySnapshot：新增 CustomHeaders、CustomBody
...
- OverrideUrl = null
- ModelSnapshot.CustomHeaders = null
- ModelSnapshot.CustomBody = null
- ModelKeySnapshot.CustomHeaders = null
- ModelKeySnapshot.CustomBody = null

理由：

- 旧库没有这些概念
- 不能从当前 Host、provider 或业务代码可靠反推出模型级 override
- 不能从当前 Host、provider 或业务代码可靠反推出 key 级 custom headers / custom body
- CustomHeaders 的目标语义固定为多行文本，不按 JSON 解析
- CustomBody 的目标语义固定为 JSON patch object，不是完整 body 覆盖
...
1. 扩表，新增并存字段
2. 回填 ModelSnapshot 的字符串能力字段，以及 ModelSnapshot / ModelKeySnapshot 的新配置字段
...
### 4. CustomHeaders / CustomBody 需要做格式约束

虽然数据库先存 VARCHAR(MAX)，但最终接口与服务层仍应做格式限制，而不是接受任意文本。

其中：

- CustomHeaders 应按多行文本解释，每行一个 header。
- CustomBody 应限制为 JSON patch object，而不是任意 JSON，也不是完整 body 覆盖。
...
2. `Effort` 存 null，null 表示未指定。
3. 压缩字段统一改名为 `Compression`，语义固定为 0-100，不再增加模型级压缩区间字段。
4. 输出格式支持列表改名为 `SupportedFormats`，只存后缀名列表，例如 jpg,png,webp。
5. `CustomHeaders` 采用多行文本。
6. `CustomBody` 的语义固定为 JSON patch object。
7. `ModelKeySnapshot` 也增加 `CustomHeaders` / `CustomBody`，且两列都允许为 null。

## 结论

这次迁移的核心不是“多加几个列”，而是把新增能力放在正确的层级：

- ModelKeySnapshot 负责 key 级连接信息与传输层补充字段
- ModelSnapshot 负责模型能力与模型级传输层
