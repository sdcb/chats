    ...
    public async Task UpdateUsage(ChatsDB db, int userModelId, CancellationToken cancellationToken)
    {
        await db.UserModels
            ...
            .ExecuteUpdateAsync(userModel => userModel
                .SetProperty(p => p.TokenBalance, v => db.UsageTransactions.Where(x => x.ModelSnapshot.ModelId == v.ModelId && x.CreditUserId == v.UserId).Sum(x => x.TokenAmount))
                .SetProperty(p => p.CountBalance, v => db.UsageTransactions.Where(x => x.ModelSnapshot.ModelId == v.ModelId && x.CreditUserId == v.UserId).Sum(x => x.CountAmount)), cancellationToken);
    }
}