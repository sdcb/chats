...
| `allowStreaming` | `boolean` | 模型是否支持流式响应 | 从 ModelReference 迁移 |
...
| `maxResponseTokens` | `number` | 模型的最大响应 Token 数（输出 Token 限制） |

##### 推理级别字段

| 字段 | 类型 | 说明 | 枚举值 |
|------|------|------|--------|
| `reasoningEffortOptions` | `number[]` | 支持的推理级别选项 | `0`=Default（默认）<br>`1`=Minimal（最小）<br>`2`=Low（低）<br>`3`=Medium（中）<br>`4`=High（高） |
...
| 字段 | 类型 | 说明 | 枚举值 |
|------|------|------|--------|
| `apiType` | `number` | API 类型 | `0` = ChatCompletion API（默认）<br>`1` = Response API（如 o3-pro） |
| `useAsyncApi` | `boolean` | 是否使用异步 API | `true` = 异步模式（需轮询结果）<br>`false` = 同步模式（默认） |

**说明**：`o3-pro` 模型需要同时设置 `apiType=1` 和 `useAsyncApi=true`。
...
1. **表单字段**：为模型编辑表单添加上述 18 个新字段的输入控件
2. **条件显示**：
   - 根据 `ModelProvider` 显示/隐藏特定字段（如 `thinkTagParserEnabled` 仅 DeepSeek 相关）
   - `apiType` 和 `useAsyncApi` 通常配套使用
3. **验证规则**：
...
4. **创建模型时的默认值建议**(常规通用值,管理员可后续在编辑对话框中调整):
   ...
   - **温度范围**:`minTemperature=0.0, maxTemperature=2.0`
   - **Token 限制**:`contextWindow=128000, maxResponseTokens=4096`
   - **推理级别**:`reasoningEffortOptions=[]`(空数组,表示不支持)
   - **图像尺寸**:`supportedImageSizes=[]`(空数组,表示不支持)
   - **API 类型**:`apiType=0, useAsyncApi=false`
   - **Token 字段**:`useMaxCompletionTokens=false`
   - **遗留标记**:`isLegacy=false`
...
3. **参数限制**：
   - 温度滑块范围：`minTemperature` 到 `maxTemperature`
   - 推理级别下拉框：根据 `reasoningEffortOptions` 生成选项（空数组表示不支持）
4. **图像尺寸**：图像生成模型根据 `supportedImageSizes` 提供尺寸选择器（空数组表示不支持）
...
**A**:
- `apiType`: 区分 API 端点类型
  - `0` (ChatCompletion): 标准聊天完成 API（同步或流式）
  - `1` (Response): 响应式 API（如 o3-pro 的响应端点）
- `useAsyncApi`: 区分调用模式
  - `false`: 同步调用，等待响应返回
  - `true`: 异步调用，需要轮询结果
