<!-- Language: zh-CN -->
<p align="right"><a href="../../en-US/release-notes/1.8.0.md">English</a> | <b>简体中文</b></p>

# Chats 1.8.0 发布说明

> 发布日期：2025-11-03（相对 1.7.2 以来 27 次提交）
...
- 初始只加载提供商列表（减少 90% 初始加载时间）
- 展开时异步加载子层级数据
- 加载中显示骨架屏（基于后端返回的数量）

**提供商查询优化**：
- 单个 EF Core 查询替代 3+ 次数据库调用
...
- ❌ 删除 `SkiaSharp`（不再需要）
- ❌ 删除 `SkiaSharp.NativeAssets.Linux.NoDependencies`（不再需要）
