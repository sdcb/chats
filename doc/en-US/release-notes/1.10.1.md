<!-- Language: en-US -->
<p><a href="./README.md">📋 All Versions</a> <span style="float:right"><b>English</b> | <a href="../../zh-CN/release-notes/1.10.1.md">简体中文</a></span></p>
...
- 🏷️ **Rename**: `Session Manager` is renamed to `Sandbox Manager`.
- 📋 **Session info card**: New `SessionInfoCard` with image/CPU/memory/pid/network/IP/create-expire time details and touch refresh.
- 🌍 **Environment variables editor**: New `SessionEnvVarEditor` with table/raw modes, validation, duplicate checks, and unsaved-change guards. Backend adds `GET/PUT /{encryptedSessionId}/environment-variables`.
- 📂 **File management performance**: Directory listing switches to `ls -la --full-time` command path instead of archive listing; adds `DockerOutputParser` for Linux/Windows output.
- 🗑️ **Delete session**: New endpoint and UI flow for deleting Docker sessions.
- 🔗 **OwnerChatId binding**: `ChatDockerSession` now supports direct Chat binding via `OwnerChatId`, enabling session creation before any turn exists.
- 📊 **Tabbed layout**: Info / Env / Command / Files / Editor with state retention across tab switches.
- 🐳 **Image pull progress**: Better progress visibility when pulling images.
- 🛠️ **Bug fixes**: Session termination sync, duplicate file loading, drag-upload issues, and download path issues.
- 📦 **Default image strategy update**: `CodeInterpreter:DefaultImage` defaults to `sdcb/code-interpreter:latest`.

### 2) Server-side ETag Caching

- ⚡ **EtagCacheHelper**: Adds `XxHash128`-based ETag generation and `304 Not Modified` handling.
- 📡 **Applied to high-frequency APIs**: `GET /api/models`, `GET /api/prompts` (brief/default), `GET /api/messages/{chatId}`, `GET /api/user-chats/groups`.
- 🗑️ **Frontend cache removal**: Removes `chatCache.ts` localStorage chat cache; cache consistency is now driven by server ETag.

### 3) ChatMiniMap Navigation

- 🗺️ **New `ChatMiniMap`**: Right-side message mini-map for fast navigation by user/assistant message distribution.
- ⬆️⬇️ **Scroll controls moved**: Top/bottom/previous-user-message controls are moved out of `ChatInput` into `ChatMiniMap`.

### 4) API Key Management Improvements

- 🔐 **Creation flow**: New `ApiKeyDialog` supports name + expiry inputs and one-time full key display after creation.
- ✏️ **Edit support**: API key comment and expiry can be updated.
- 📋 **Bulk copy**: Adds "copy all keys".
- 🔒 **Backend safety**: List API returns masked keys only (`ToMasked()`); creation uses `CreateApiKeyDto` (`comment`, `expires`).

### 5) File Download Experience

- 📎 **Content-Disposition**: Aliyun OSS / AWS S3 / Azure Blob / local file service now preserve original filename for downloads.
- 🔧 **Local service fix**: Fixes missing Content-Disposition in local file downloads.
...
- Adds font size setting (12–18px slider + input), persisted in localStorage via `--chat-font-size`.
